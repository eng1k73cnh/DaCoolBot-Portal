	...
	return (
		<header className="navbar container mx-auto px-4 pt-5">
			<div className="navbar-start">
				<Image src="/icon.png" alt="DaCoolBot Logo" width={50} height={50} className="rounded-full mr-4" />
				<span className="self-center text-2xl font-semibold whitespace-nowrap hover:text-blue-500 transition-colors hidden lg:block">DCB Portal</span>
			</div>
			<div className="navbar-end space-x-4">
				{!session?.user ? (
					...
					<>
						...
						<img src={session.user.image || ""} alt="User Image" className="rounded-full h-10 w-10" />
						<div>
							<span className="self-center text-xl font-medium whitespace-nowrap">{session.user.name}</span>
							{session.user.discriminator !== "0" && (
								<span className="self-center text-base font-light whitespace-nowrap">#{session.user.discriminator}</span>
							)}
						</div>
